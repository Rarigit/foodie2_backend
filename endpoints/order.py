...
# Only did is-cancelled input because it was a client side insert and clients can only cancel an order
...
@app.post('/api/order')
def insert_order():
    required_data = ['restaurantId', 'items']
    ...
    item_input = ",".join(map(str, request.json.get('items', [])))
    result = run_statement("CALL insert_order_arg_tkresid(?,?,?,?)", [token_input, client_id_input, restaurant_id_input, item_input])
    if result == None:
        return make_response(jsonify("Order inserted successfully"), 200)
    else:
        return make_response(jsonify("Failed to insert order. Something went wrong"), 500)
        # print(is_cancelled_input, type(is_cancelled_input))
                
# THIS IS NONSENSE. DATA TRUNCATION ERRORS EVERYWHERE
